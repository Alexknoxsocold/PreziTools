...
function YesterdayResults() {
  ...
  const marketStats=Object.values((data?.outcomes??[]).reduce<Record<string,{label:string;wins:number;total:number}>>((acc,row)=>{
    const key=`${row.sport}::${row.market}`;
    const current=acc[key]??{label:`${row.sport} · ${row.market}`,wins:0,total:0};
    current.total+=1;
    if(row.result==="won")current.wins+=1;
    acc[key]=current;
    return acc;
  },{}));
  const topMarket=marketStats
    .filter(x=>x.total>=2)
    .map(x=>({...x,rate:Math.round((x.wins/x.total)*100)}))
    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-1.5">
      <button type="button" onClick={()=>setOpen(v=>!v)} className="w-full rounded-xl border border-border/60 bg-card/70 px-3 py-2.5 text-left transition-colors hover:bg-muted/40" aria-expanded={open}>
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex shrink-0 items-center gap-2">{query.isLoading?<span className="text-[10px] text-muted-foreground">Loading…</span>:data&&decided>0?<><span className="font-mono text-xs font-black"><span className="text-emerald-500">{data.wins}W</span> <span className="text-muted-foreground">–</span> <span className="text-rose-500">{data.losses}L</span></span>{rate!==null&&<span className="rounded-full border border-border/60 bg-muted/45 px-2 py-1 font-mono text-[9px] font-black text-foreground">{rate}% HIT</span>}</>:<span className="text-[10px] text-muted-foreground">No graded plays</span>}<ChevronDown className={cn("h-4 w-4 text-muted-foreground transition-transform",open&&"rotate-180")}/></div>
        </div>
      </button>
      {open&&<div className="mt-1.5 overflow-hidden rounded-xl border border-border/60 bg-card/80 shadow-lg"><div className="border-b border-border/50 px-3 py-2"><div className="flex items-center justify-between gap-3"><span className="text-[9px] font-bold uppercase tracking-[.16em] text-muted-foreground">{data?.date??"Yesterday"}</span>{data&&<span className="text-[9px] text-muted-foreground">{data.total} verified play{data.total===1?"":"s"}</span>}</div>{topMarket&&<div className="mt-1.5 flex items-center justify-between gap-2 rounded-lg border border-emerald-500/15 bg-emerald-500/[0.05] px-2.5 py-1.5"><span className="truncate text-[9px] font-bold text-muted-foreground">Best hit rate · {topMarket.label}</span><span className="shrink-0 font-mono text-[9px] font-black text-emerald-500">{topMarket.rate}% <span className="text-muted-foreground">({topMarket.wins}/{topMarket.total})</span></span></div>}</div>{query.isLoading?<div className="p-5 text-center text-xs text-muted-foreground">Loading verified results…</div>:query.error?<div className="p-5 text-center text-xs text-muted-foreground">Yesterday&apos;s results are temporarily unavailable.</div>:data?.outcomes?.length?<div className="max-h-[340px] divide-y divide-border/40 overflow-y-auto">{data.outcomes.map(row=><Link key={row.id} href={row.href}><div className="grid grid-cols-[52px_1fr_auto] items-center gap-2 px-3 py-3 transition-colors hover:bg-muted/35"><div className="text-[9px] font-black text-muted-foreground">{row.sport}</div><div className="min-w-0"><div className="truncate text-xs font-bold">{row.pick}</div><div className="mt-0.5 truncate text-[9px] text-muted-foreground">{row.market} · {row.matchup}</div><div className="mt-0.5 truncate text-[9px] text-muted-foreground/80">{row.actual}</div></div><div className="flex items-center gap-1.5"><span className="rounded-md border border-border/60 bg-background/55 px-1.5 py-1 font-mono text-[9px] font-black text-muted-foreground">{Number(row.probability).toFixed(1)}%</span>{row.result==="won"?<span className="inline-flex items-center gap-1 rounded-full border border-emerald-500/25 bg-emerald-500/10 px-2 py-1 text-[9px] font-black text-emerald-500"><CheckCircle2 className="h-3 w-3"/>WIN</span>:<span className="inline-flex items-center gap-1 rounded-full border border-rose-500/25 bg-rose-500/10 px-2 py-1 text-[9px] font-black text-rose-500"><XCircle className="h-3 w-3"/>LOSS</span>}</div></div></Link>)}</div>:<div className="p-5 text-center"><div className="text-xs font-semibold">No verified graded plays for yesterday yet.</div><div className="mt-1 text-[10px] text-muted-foreground">Only locked, actually graded predictions appear here.</div></div>}</div>}
    </div>
  </div>;
}
  ...
  return (
    <nav className="site-sports-nav border-b bg-card sticky top-14 z-40" aria-label="Primary sports navigation">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        <div className="flex items-center gap-1 overflow-x-auto overscroll-x-contain py-1.5 scrollbar-none md:justify-center">
          {navItems.map((item) => {
