...
export default function InternationalBestPlays(){
  ...
  const plays=useMemo(()=>{
    const rows:{id:string;league:"NPB";market:string;matchup:string;pick:string;probability:number;edge:number;status:Status;time:string}[]=[];
    for(const g of q.data?.games??[]){
      if(g.league!=="NPB") continue;
      for(const [market,p] of [["Moneyline",g.moneyline],["Over / Under",g.total]] as const){
        if(!p||p.status==="NO_PLAY")continue;
        rows.push({id:`${g.id}-${market}`,league:"NPB",market,matchup:`${g.awayTeam} @ ${g.homeTeam}`,pick:market==="Over / Under"&&p.line!=null?`${p.pick} ${p.line}`:p.pick,probability:p.probability,edge:p.edge,status:p.status,time:g.startTime});
      }
    }
  ...
  return <section className="rounded-xl border bg-card/80 p-3 sm:p-4">
    <div className="mb-3 flex items-center justify-between gap-3"><div><div className="text-sm font-black">NPB Best Plays</div><div className="text-[10px] text-muted-foreground">Official-data model vs live market · KBO withheld while model is still being validated</div></div><Link href="/kbo-npb" className="text-[10px] font-bold text-primary">View all</Link></div>
