...
export default function BestPlaysHub() {
  ...
  return <div className="relative left-1/2 w-screen -translate-x-1/2 -mt-4 min-h-[calc(100vh-10rem)] overflow-hidden">
    ...
    <div className="relative z-10 mx-auto max-w-7xl space-y-4 px-3 py-4 sm:px-4 md:px-6 lg:px-8">
      <div className="relative z-20 overflow-hidden rounded-xl border bg-card/80 shadow-sm backdrop-blur-sm">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-card via-card/95 to-card/55"/><div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-card/75"/>
        <div className="relative flex flex-wrap items-center justify-between gap-3 p-4 sm:p-5"><div className="min-w-0"><div className="flex items-center gap-2"><Trophy className="w-4 h-4 text-primary"/><span className="text-sm font-bold">Best Plays</span></div><div className="mt-1 text-[10px] text-muted-foreground">Daily results reset at midnight ET.</div></div><div className="relative"><select value={view} onChange={e=>setView(e.target.value as View)} className="appearance-none rounded-md border bg-background/90 pl-3 pr-9 py-2 text-xs font-semibold shadow-sm backdrop-blur focus:outline-none focus:ring-2 focus:ring-ring" aria-label="Best Plays view"><option value="games">Today's Games</option><option value="outcomes">Today's Outcomes</option><option value="wins">Today's Winning Outcomes</option></select><ChevronDown className="pointer-events-none absolute right-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/></div></div>
      </div>

      <div className="rounded-lg border bg-card/80 p-3 sm:p-4 backdrop-blur-sm"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex items-center gap-2 text-sm font-bold"><Mail className="h-4 w-4 text-primary"/>PreziTools Daily Plays</div><div className="mt-1 text-[10px] text-muted-foreground">Get WNBA, NBA, MLB and NPB strongest plays by email. KBO is withheld while the model is still being validated. Unsubscribe anytime.</div></div><form onSubmit={subscribeNewsletter} className="flex w-full gap-2 sm:w-auto"><input type="email" required value={newsletterEmail} onChange={e=>setNewsletterEmail(e.target.value)} placeholder="you@email.com" className="min-w-0 flex-1 rounded-md border bg-background px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-ring sm:w-56" aria-label="Newsletter email"/><button type="submit" disabled={subscribing} className="rounded-md bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground disabled:opacity-60">{subscribing?'Joining…':'Join'}</button></form></div>{newsletterStatus?<div className="mt-2 text-[10px] text-muted-foreground">{newsletterStatus}</div>:null}</div>
