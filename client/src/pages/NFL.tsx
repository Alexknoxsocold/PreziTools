...
function PlayerPick({ row, index, game, locked }: { row: PlayerMarket; index: number; game: NflGame; locked: boolean }) {
  ...
  return <div className={`rounded-xl border p-3 backdrop-blur-sm sm:p-4 ${won ? 'nfl-verified-winner border-emerald-400 bg-emerald-500/10' : row.qualifies ? 'border-emerald-500/30 bg-background/75 ring-1 ring-emerald-500/10' : 'bg-background/65'}`}><div className="flex gap-2.5 sm:gap-4"><PlayerPhoto row={row}/><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2 sm:gap-3"><div className="min-w-0"><div className="flex items-center gap-1.5 sm:gap-2"><h3 className="truncate text-sm font-black sm:text-base">{row.player}</h3>{won ? <Badge className="h-5 border-emerald-300/60 bg-emerald-500 px-1.5 text-[7px] text-white">WON</Badge> : row.qualifies && <Badge className="hidden h-5 border-emerald-500/30 bg-emerald-500/15 px-1.5 text-[7px] text-emerald-500 xs:inline-flex">{locked ? 'PLAY' : 'LEADING'}</Badge>}{dataNotReady && <Badge className="hidden h-5 border-amber-500/30 bg-amber-500/15 px-1.5 text-[7px] text-amber-400 xs:inline-flex">DATA NOT READY</Badge>}</div><div className="mt-1 flex min-w-0 items-center gap-1.5 sm:gap-2"><TeamMark team={team}/><span className="truncate text-[8px] text-muted-foreground sm:text-[9px]">{row.position || 'Skill'} • {row.quoteCount} books</span></div></div><div className="shrink-0 text-right"><div className="font-mono text-xl font-black sm:text-2xl">{odds(row.bestOdds)}</div><div className="text-[7px] uppercase text-muted-foreground sm:text-[8px]">best odds</div></div></div><div className="mt-2.5 grid grid-cols-4 gap-1.5 border-t pt-2.5 sm:mt-3 sm:gap-3 sm:pt-3"><Metric label="Prezi %" value={pct(row.modelProbability)} good={row.qualifies}/><Metric label="Book %" value={pct(row.impliedProbability)}/><Metric label="Edge pts" value={edge == null ? '—' : `${edge >= 0 ? '+' : ''}${edge.toFixed(1)}`} good={edge != null && edge > 0}/><Metric label="Bet EV" value={ev(row.expectedValue)} good={(row.expectedValue ?? 0) > 0}/></div></div></div><QuoteStrip quotes={row.quotes}/>{row.reasons?.length ? <details className="mt-2.5 sm:mt-3"><summary className="cursor-pointer text-[8px] font-bold uppercase tracking-wide text-muted-foreground sm:text-[9px]">{dataNotReady ? 'Why this is not a model pick' : row.qualifies ? 'Why this qualifies' : 'Why the model is watching this player'}</summary><div className="mt-2 rounded-lg bg-background/55 p-2.5 text-[8px] leading-4 text-muted-foreground sm:p-3 sm:text-[9px] sm:leading-5">{row.reasons.slice(1).map((r, i) => <div key={i}>• {r}</div>)}</div></details> : null}</div>;
}
