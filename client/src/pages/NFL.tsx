...
type NflGame = { id: string; date: string; status: string; away: Team; home: Team; marketStatus: 'available' | 'unavailable'; marketAvailable?: Readiness; modelReady?: Readiness; moneyline: { away: MoneylineSide; home: MoneylineSide } | null; anytimeTd: PlayerMarket[]; firstTd: PlayerMarket[]; qualified: Readiness };
...
function MoneylineCard({ game }: { game: NflGame }) {
  const choices = game.moneyline ? [game.moneyline.away, game.moneyline.home].filter(x => x.model?.qualifies) : [];
  const official = choices.length > 0;
  const dataNotReady = game.marketAvailable?.moneyline !== true || game.modelReady?.moneyline !== true;
  return <article className="relative isolate overflow-hidden rounded-xl border bg-card shadow-sm sm:rounded-2xl"><StadiumBackdrop home={game.home}/><GameHeader game={game} qualified={official} dataNotReady={dataNotReady}/><div className="relative z-10 space-y-2.5 p-3 sm:space-y-3 sm:p-5">{official ? choices.map(s => <div key={s.team} className="rounded-xl border bg-background/65 p-3 backdrop-blur-sm sm:p-4"><div className="flex justify-between"><b>{s.team}</b><b className="font-mono text-lg sm:text-xl">{odds(s.bestOdds)}</b></div></div>) : <div className={`rounded-xl border bg-background/65 p-6 text-center text-xs font-black backdrop-blur-sm sm:p-8 sm:text-sm ${dataNotReady ? 'text-amber-400' : 'text-muted-foreground'}`}>{dataNotReady ? 'DATA NOT READY' : 'NO OFFICIAL PLAY'}</div>}</div></article>;
}
