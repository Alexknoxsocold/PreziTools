...
const STRONGEST_COPY =
  "Only each game's #1 Best Play and #2 Strong Play appear when model probability is at least 10%. Market Value remains separate and only appears when real odds clear the edge/EV threshold.";
    ...
    const cleanHiddenWnbaUi = () => {
      for (const el of Array.from(root.querySelectorAll("div,p,h1,h2,h3,h4"))) {
        ...
        while (block && block !== root) {
          ...
          if (
            blockText.includes(STRONGEST_TITLE) &&
            blockText.includes(STRONGEST_COPY) &&
            blockText.length < 700
          ) {
