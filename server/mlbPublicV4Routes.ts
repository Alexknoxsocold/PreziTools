...
import { activeOvernightSlateDateKey, freezeAndMergeMlbNrfi } from "./frozenDisplayAdapters";
...
export function registerMlbV4PublicRoutes(app:Express):void{
  // Register HR diagnostics; snapshot capture is owned by the HR response route.
  registerMlbHrCalibration(app);
  app.get("/api/mlb/nrfi",async(req,res)=>{try{const requestedDate=typeof req.query.date==="string"?req.query.date:undefined;if(requestedDate&&!/^\d{4}-\d{2}-\d{2}$/.test(requestedDate))return res.status(400).json({error:"date must use YYYY-MM-DD format"});const raw=await fetchNrfiDataV4Live(requestedDate??activeOvernightSlateDateKey()),enriched=await enrichMlbResponseWithEvidence(raw),data=await freezeAndMergeMlbNrfi({...enriched,modelVersion:"v4-live"});persistLiveV4(data.date,data.games);res.setHeader("Cache-Control","no-store, max-age=0");res.setHeader("X-MLB-Model-Version","v4-live");return res.json({...data,modelVersion:"v4-live",evidenceModel:"top-order-weather-shadow-v1"})}catch(error){console.error("[MLB V4 Public] Error:",error);return res.status(502).json({error:"Unable to load MLB V4 NRFI data"})}});
