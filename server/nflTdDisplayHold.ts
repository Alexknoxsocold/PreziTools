...
/** Returns frozen pregame TD candidates for the 90-minute post-kickoff hold.
...
export async function getHeldNflTdPlays(gameId:string):Promise<{anytime:HeldTdPlay[];first:HeldTdPlay[]}>{
  ...
  try{
    ...
    const r=await c.query(`SELECT market,player,model_version,model_probability,market_probability,best_odds,best_book,edge_points,expected_value,confidence,quote_count,feature_snapshot FROM nfl_td_prediction_history WHERE game_id=$1 AND model_version=$2 ORDER BY market,model_probability DESC,captured_at DESC`,[gameId,version]);
